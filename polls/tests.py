...
class Authenticationtests(APITestCase):
    ...
    def test_token_endpoint(self):
        ...
        print(user.username)
        print(user.password)
        user.StaticToken = 'AAAAAAAAAAAAAAAAAAAA'
        login = client.post('/token/',{"username":"user", "password":"password"}, format = 'json')
        print(login.data)
        # force_authenticate(l, user = user, token = user.StaticToken)
        # print(l)
...
#         force_authenticate(new_votes, user=user, token=user.auth_token)
...
#             client.post('/polls/questions/', {"id":"314","question_text":"Foobar","pub_date":"2019-05-29T10:23:26-04:00"}, format = 'json')
#
#
#
...
#
#         with self.assertRaises(ValidationError):
#             client.post('/polls/questions/', {"id":"314","question_text":"Foobar","pub_date":"2019-05-29T10:23:26-04:00","choices":[{"choice_text":"Bar"}]}, format = 'json')
#             #This checks to ensure that a ValidationError is raised, denoting that the request is bad (status 404)
